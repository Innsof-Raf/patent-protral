  ...
  MyAppointmentsBloc() : super(MyAppointmentsState.initial()) {
    on<GetMyAppointments>((event, emit) async {
      ...
      myAppointmentsFetchingOptions.fold(
        ...
        for (MyAppointmentModel appointment in myAppointments) {
          if (!monthTimeLineList.contains(DateTime(
              appointment.appointmentDateTime.year,
              appointment.appointmentDateTime.month))) {
            monthTimeLineList.add(DateTime(appointment.appointmentDateTime.year,
                appointment.appointmentDateTime.month));
          }
          if (appointment.appointmentDateTime == DateTime.now() ||
              appointment.appointmentDateTime.isAfter(DateTime.now())) {
            notConsultedAppointments.add(appointment);
            if (!monthTimeLineListOfNotConsulted.contains(DateTime(
                appointment.appointmentDateTime.year,
                appointment.appointmentDateTime.month))) {
              monthTimeLineListOfNotConsulted.add(DateTime(
                  appointment.appointmentDateTime.year,
                  appointment.appointmentDateTime.month));
            }
          } else {
            consultedAppointments.add(appointment);
            if (!monthTimeLineListOfConsulted.contains(DateTime(
                appointment.appointmentDateTime.year,
                appointment.appointmentDateTime.month))) {
              monthTimeLineListOfConsulted.add(DateTime(
                  appointment.appointmentDateTime.year,
                  appointment.appointmentDateTime.month));
            }
          }
    ...
    on<ChangeResheduledAppointmentDetails>((event, emit) {
      ...
      final int currentAppointmentIndex =
          myAppointments.indexOf(event.appointment);
    ...
    on<CancelAppointment>((event, emit) async {
      ...
      final Either<ErrorModel, Map> appointmentCancelationOptions =
          await MyAppointmentServices.cancelAppointment(
              appointmentId: event.idAppointment, token: event.token);
      appointmentCancelationOptions.fold(
          (error) => emit(state.copyWith(
              isAppointmentsCancelationFailed: false,
              error: error)), (sucessesResponse) {
        List<MyAppointmentModel> myAppointments =
            List.from(state.myAppointments);
    ...
    on<StoreBokkedApoointment>((event, emit) {
      ...
      List<MyAppointmentModel> myNotConsultedAppointments =
          List.from(state.myNotConsultedAppointments);
      List<MyAppointmentModel> myConsultedAppointments =
          List.from(state.myConsultedAppointments);
      List<DateTime> monthList = List.from(state.monthTimeLineList);
      List<DateTime> monthTimeLineListOfConsulted =
          List.from(state.monthTimeLineListOfConsulted);
      List<DateTime> monthTimeLineListOfNotConsulted =
          List.from(state.monthTimeLineListOfNotConsulted);
      if (event.appointment.appointmentDateTime.isAfter(DateTime.now())) {
        ...
        if (!monthList.contains(DateTime(
            event.appointment.appointmentDateTime.year,
            event.appointment.appointmentDateTime.month))) {
          monthList.add(DateTime(event.appointment.appointmentDateTime.year,
              event.appointment.appointmentDateTime.month));
          monthTimeLineListOfNotConsulted.add(DateTime(
              event.appointment.appointmentDateTime.year,
              event.appointment.appointmentDateTime.month));
          ...
          monthTimeLineListOfNotConsulted.sort(
            (a, b) => a.compareTo(b),
          );
        }
      } else {
        ...
        if (!monthList.contains(DateTime(
          ...
          monthTimeLineListOfConsulted.sort(
            (a, b) => a.compareTo(b),
          );
        }
      }
