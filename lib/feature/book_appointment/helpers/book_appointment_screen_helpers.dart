...
import 'package:patient_portal/feature/book_appointment/widgets/book_appointment_confirmation_popup.dart';
...
class BookAppointmentScreenHelpers {
  ...
  static bookAppointment(
    ...
    if (context.read<UserBloc>().state.user!.members.isEmpty) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(alertSnakBar(
  ...
  static ValueNotifier<DateTime> selectedDateNotifier =
      ValueNotifier(DateTime.now());
  static ValueNotifier<DateTime?> selectedSlotNotifier = ValueNotifier(null);
  static ValueNotifier<MemberModel?> selectedMemberNotifier =
      ValueNotifier(null);
  ...
  static SnackBar alertSnakBar({required String content}) {
    return SnackBar(
      content: Text(
        content,
        style:
            AppTextStyles.bodySemiBoldRoboto.copyWith(color: AppColors.white),
      ),
